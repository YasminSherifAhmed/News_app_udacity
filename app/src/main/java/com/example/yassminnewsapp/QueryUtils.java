package com.example.yassminnewsapp;
...
public final class QueryUtils {
    ...
    private static List<Data> extractFeatureFromJson(String newsJSON) {

        if (TextUtils.isEmpty(newsJSON)) {
            return null;
        }

        List<Data> newses = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                Data news = new Data(title, information, authorName, date, url);
                newses.add(news);
            }
